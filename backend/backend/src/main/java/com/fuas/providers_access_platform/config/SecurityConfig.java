    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf(csrf -> csrf.disable()) // `csrf().disable()` still works here
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/login", "/api/service-request/published/{providerId}", "/api/provider/master-agreements","api/provider/bid","/api/employees/{providerId}","/api/service-request/submit").permitAll() // Whitelist these URLs
