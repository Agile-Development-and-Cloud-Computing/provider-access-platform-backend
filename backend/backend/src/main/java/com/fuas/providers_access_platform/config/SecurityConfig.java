    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf(csrf -> csrf.disable()) // `csrf().disable()` still works here
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/login", "/api/service-request/published/{providerId}",
                                "/api/provider/role-offers","/api/employees/{providerId}","/api/service-request/submit",
                                "/api/service-request/offers", "/api/employees/update/${employeeId}/${providerId}",
                                "/api/employees/add","/api/provider/offer-response","/api/service-request/update-status",
                                "/api/service-request/published","/api/provider/master-agreements","/api/provider/bid",
                                "/api/provider/edit-credentials","/api/provider/get-user").permitAll() // Whitelist these URLs
